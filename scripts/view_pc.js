// Coach Beurt V50.98 — PC export IA actions utiles
...
function pcPlannedExerciseLine(e, session){
  var parsed=parseTargetReps(e.format,10);var target=parsed.min||parsed.max||10;
  var shown=athleteSuggestedLoad(e.name,e.load,target);
...
function pcDayAuditLines(day, week){
  ...
  (w.blocks||[]).forEach(function(block){
    ...
    if(block.exercises&&block.exercises.length){
      block.exercises.forEach(function(e){lines.push(pcPlannedExerciseLine(e,session));});
    }else if(block.text){
  ...
  if(next===wk){
    ...
    days.forEach(function(day){
      ...
      (w.blocks||[]).forEach(function(block){
        ...
        (block.exercises||[]).forEach(function(e){
          var parsed=parseTargetReps(e.format,10);var target=parsed.min||parsed.max||10;
          lines.push("  • "+e.name+" · "+(e.format||"?")+" · suggéré "+athleteSuggestedLoad(e.name,e.load,target)+(e.rest?" · repos "+e.rest:""));
        });
