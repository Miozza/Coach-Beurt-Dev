...
function gfBlocks(day, week){
  ...
  if(day === "lundi") return [
    { time:"8 min", title:"Warm-up hanches / fessiers", tag:"Préparation", kind:"warmup",
      text:"Bike facile 3 min + 90/90 hip switch 8/côté + glute bridge 2×12 avec pause + lateral band walk 2×10/côté + 2-3 ramp-up sets du hip thrust." },
    ...
    { time:"13 min", title:"B. Amplitude longue sans ego", tag:"Hypertrophie", kind:"accessory",
      exercises:[
        gfEx("B1. Front-Foot Elevated Split Squat", p.supportSets + "×" + p.repSupport + "/jambe", "modéré, " + p.rir, "0:30 avant B2", "Grand pas. Descente contrôlée. Tu dois sentir l'étirement fessier, pas chercher un record."),
        gfEx("B2. Cable Pull-Through", p.supportSets + "×12-15", "léger à modéré", "1:15 après B2", "Hinge propre. Les bras ne tirent pas.")
      ] },
  ...
  if(day === "mardi") return [
    ...
    { time:"14 min", title:"B. Dos / posture", tag:"Secondaire", kind:"accessory",
      exercises:[
        gfEx("B1. Chest-Supported Row", deload ? "2×10" : "3×10-12", "modéré", "0:30 avant B2", "Support poitrine pour ne pas charger le bas du dos."),
        gfEx("B2. Face Pull", deload ? "2×15" : "3×15-20", "léger", "1:00 après B2", "Scapulas arrière/bas, cou relax.")
      ] },
  ...
  if(day === "jeudi") return [
    ...
    { time:"13 min", title:"B. Unilatéral contrôlé", tag:"Hypertrophie", kind:"accessory",
      exercises:[
        gfEx("B1. Step-Up haut contrôlé", p.supportSets + "×" + p.repSupport + "/jambe", "modéré", "0:30 avant B2", "Pousser par la jambe sur la box. Pas d'élan avec la jambe arrière."),
        gfEx("B2. Single-Leg Hip Thrust", p.supportSets + "×10-12/jambe", "poids du corps ou DB légère", "1:15 après B2", "Contrôle du bassin. Contraction propre.")
      ] },
  ...
  return [
    ...
    { time:"14 min", title:"B. Carries + tronc", tag:"Core", kind:"accessory",
      exercises:[
        gfEx("B1. Farmer Carry", deload ? "3×30 sec" : "4×30-40 sec", "lourd mais propre", "0:30 avant B2", "Tronc solide. Pas de compensation lombaire."),
        gfEx("B2. Pallof Press", deload ? "2×10/côté" : "3×10-12/côté", "modéré", "1:00 après B2", "Anti-rotation. Bassin stable.")
      ] },
